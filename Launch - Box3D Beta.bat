@echo off
title Marble Game Box3D Beta
setlocal
set PORT=5174
set URL=http://localhost:%PORT%/?physics=box3d
...
echo [INFO] Launching Box3D Beta at %URL%
if not exist public\box3d\bridge-manifest.json (
    echo [WARNING] Box3D beta assets are not present at public\box3d\bridge-manifest.json
    echo [WARNING] See docs\BOX3D_BETA_HANDOFF.md before expecting live physics on the beta route.
)
